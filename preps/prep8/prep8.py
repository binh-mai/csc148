...
class Tree:
    ...
    def num_negatives(self) -> int:
        ...
        >>> t5 = Tree(-11, [Tree(0, []), Tree(10, [Tree(4, [Tree(-3, [])])])])
        >>> t5.num_negatives()
        2
        """
        if self.is_empty():
    ...
    def maximum(self: Tree) -> int:
        ...
        >>> t3.maximum()
        100
        >>> t5 = Tree(1, [Tree(22, [Tree(200, [Tree(500, [])])]), Tree(100, []), Tree(30, [])])
        >>> t5.maximum()
        500
        """
        if self.is_empty():
            ...
            max_num = self._root
            for subtree in self._subtrees:
                max_num = max(max_num, subtree.maximum())
            return max_num
    ...
    def height(self: Tree) -> int:
        ...
        >>> t5 = Tree(1, [Tree(-2, [Tree(-2, [Tree(-2, [Tree(-2, [])])])]), Tree(10, [Tree(-2, [])])])
        >>> t5.height()
        5
        """
        if self.is_empty():
            ...
            tree_height = []
            for subtree in self._subtrees:
                tree_height.append(subtree.height())
            return max(tree_height) + 1

    def __contains__(self, item: Any) -> bool:
        """Return whether this tree contains <item>.

        >>> t = Tree(1, [Tree(-2, []), Tree(10, [Tree(0, [Tree(100, [])])]), Tree(-30, [])])
