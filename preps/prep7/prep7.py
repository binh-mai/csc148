...
def nested_max(obj: Union[int, list]) -> int:
    ...
    if isinstance(obj, int):
        ...
        lst_num = []
        for sublist in obj:
            lst_num.append(nested_max(sublist))
        return max(lst_num)
...
def max_length(obj: Union[int, list]) -> int:
    ...
    5
    >>> max_length([1, 2, [1, 2, [3, 4, 5, 6, 7]], 4])
    5
    >>> max_length([[],1, 2, [1, 2, [3], 4, 5], 4, [1, 2]])
    6
    """
    if isinstance(obj, int):
        return 0
        ...
        len_list = []
        len_obj = len(obj)
        len_list.append(len_obj)
        for sublist in obj:
            len_list.append(max_length(sublist))
        return max(len_list)
