...
class Instructor:
    """An instructor that has certificates
    and teach workout classes in the gym.

    === Public Attributes ===
    name: Instructor's name

    === Private Attributes ===
    _cert: Instructor's certifications
    _id: Instructor's id
    """
    name: str
    _id: int
    ...
    def __init__(self, _id: int, name: str) -> None:
        """Initialize a new Instructor's roster that contains
        the key as instructor's id and value as instructor's name.
    ...
    def add_certificate(self, new_cert: str) -> bool:
        """Add certificates of an instructor to the _cert list.
        ...
        >>> instructor.add_certificate(cert)
        False
        >>> instructor.get_certificates() == ['Cardio 1']
        True
        """
        if new_cert in self._cert:
            return False
        else:
            self._cert.append(new_cert)
            return True
    ...
    def get_certificates(self) -> list:
        """Get a list of certificates of the instructors.
...
class Gym:
    ...
    _schedule: dict[datetime, dict[str,
    ...
    def add_instructor(self, instructor: Instructor) -> bool:
        ...
        instr_id = instructor.get_id()
        if instr_id in self._instructors:
            return False
        else:
            self._instructors[instr_id] = instructor
            return True
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        cert_list = workout.get_required_certificates()
        instr_cert = instructor.get_certificates()

        instr_qualified = bool
        instr_available = bool
        room_availability = bool

        if set(cert_list).issubset(instr_cert):
            instr_qualified = True

        class_info = (instructor, workout, [])
        if time_point not in self._schedule:
            instr_available = True
            room_availability = True
            if instr_qualified is True:
                self._schedule[time_point] = {}
                self._schedule[time_point][room_name] = class_info
                return True

        elif time_point in self._schedule and \
                room_name in self._schedule[time_point]:
            room_availability = False
        elif time_point in self._schedule and \
                room_name not in self._schedule[time_point]:
            room_availability = True
            for rooms in self._schedule[time_point]:
                if instructor in self._schedule[time_point][rooms]:
                    instr_available = False
                else:
                    instr_available = True

        if instr_qualified is True and instr_available is True \
                and room_availability is True:
            self._schedule[time_point][room_name] = class_info
            return True
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        """
        workout = self._workouts[workout_name]
        capacity_dict = self._room_capacities
        available_dict = {}

        for room_name in self._schedule[time_point]:
            client_list = self._schedule[time_point][room_name][2]
            if client in client_list:
                return False
            if workout in self._schedule[time_point][room_name]:
                space_available = capacity_dict[room_name] - len(client_list)
                available_dict[room_name] = space_available

        max_room = max(available_dict, key=available_dict.get)
        if available_dict[max_room] > 0:
            self._schedule[time_point][max_room][2].append(client)
            return True
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        if self._instructors == {} or (time1 not in time_list):
            return total_hour

        if time1 in time_list:
        ...
        for instr_id in self._instructors:
            instr_class[instr_id] = []
            total_hour[instr_id] = 0

        for time in time_list:
            for room in self._schedule[time]:
                instr_id = self._schedule[time][room][0].get_id()
                instr_class[instr_id].append(self._schedule[time][room][1])

        for instr_id in self._instructors:
            total_hour[instr_id] = len(instr_class[instr_id])
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        if time1 not in self._schedule:
            return []
        ...
        for instr_id in self.instructor_hours(time1, time2):
            ...
            instr_hr = self.instructor_hours(time1, time2)[instr_id]
            pay = instr_hr * (base_rate
                              + len(instr.get_certificates()) * BONUS_RATE)
            instr_pay = (instr_id, instr_name, instr_hr, float(pay))
    ...
    def _is_instructor_name_unique(self, instructor: Instructor) -> bool:
        ...
        for instr_id in self._instructors:
            if instr_name in self._instructors[instr_id].name:
                count_name += 1
    ...
    def offerings_at(self, time_point: datetime) -> list[dict[str, str | int]]:
        ...
        room_list = sorted(self._schedule[time_point])
        for room in room_list:
            instr = self._schedule[time_point][room][0]
            if self._is_instructor_name_unique(instr):
                instr_name = instr.name
            else:
                instr_name = instr.name + " (" + str(instr.get_id()) + ")"
            workout_name = self._schedule[time_point][room][1].name
            client_num = len(self._schedule[time_point][room][2])
            capacity = self._room_capacities[room]
            seats_left = capacity - client_num

            offer = create_offering_dict(date, time, workout_name, room,
                                         client_num, seats_left, instr_name)
    ...
    def to_schedule_list(self, week: datetime = None) \
        ...
        """
        schedule_list = []
        sorted_times = sorted(self._schedule)
        for time_point in sorted_times:
            if week is not None:
                if in_week(time_point, week):
                    schedule_list += self.offerings_at(time_point)
