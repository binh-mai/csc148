...
class NumericQuestion(Question):
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        """
        if isinstance(answer, Answer) and isinstance(answer.content, int) and \
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        value = difference / (self._max - self._min)
        return 1.0 - value
...
class YesNoQuestion(Question):
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        this question.
        """
        if isinstance(answer, Answer) and \
...
class CheckboxQuestion(MultipleChoiceQuestion):
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        """
        # ans = answer.content
        # return isinstance(ans, list) and ans != [] and len(ans) != 0 and \
        #     len(ans) == len(set(ans)) \
        #     and (set(ans)).issubset(set(self._options))

        if isinstance(answer, Answer) is False and \
                isinstance(answer.content, str) is False:
            return False

        if len(answer.content) == 0:
            return False
        for items in answer.content:
            count = 0
            for things in answer.content:
                if items == things:
                    count += 1
            if count > 1:
                return False
...
class Survey:
    ...
    def score_students(self, students: list[Student]) -> float:
        ...
        for questions_id in self._questions:
            ...
            # if criteria.score_answers(question, answer_list) is \
            #         InvalidAnswerError:
            #     return 0
            try:
