...
class Grouping:
    ...
    def add_group(self, group: Group) -> bool:
        ...
        for groups in self._groups:
            groups_list = groups.get_members()
            for students in groups_list:
                if students.id in number_ids:
                    return False
...
class AlphaGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        """
        student_list = deepcopy(list(course.get_students()))
        students = sort_students(student_list, 'name')
        sliced_list = slice_list(students, self.group_size)
        group_list = []
        for items in sliced_list:
            group_list.append(Group(items))
        grouping = Grouping()
        for groups in group_list:
            grouping.add_group(groups)
        return grouping
...
class GreedyGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        """
        lst_student = list(course.get_students())
        student_no_group = lst_student.copy()

        group_list = []
        while len(student_no_group) != 0:
            gp = []
            if len(gp) == 0:
                gp.append(student_no_group.pop(0))
            if len(gp) != 0:
                i = 0
                while i < (self.group_size - 1) and len(student_no_group) != 0:
                    gp.append(find_best_addition_to_group(survey, gp,
                                                          student_no_group))
                    i += 1
...
class SimulatedAnnealingGrouper(Grouper):
    ...
    === Private Attributes ===
    _iterations: the number of times the iteration run

    _initial temperature: the inital teperature to create groups within the
    group_size limit
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        Preconditions:
            - <course> has more students than this Grouper's group_size
        """
        # student = deepcopy(list(course.get_students()))
        # ini_groups = student
        # init_groups = slice_list(ini_groups, self.group_size)
        # best_score = total_score(survey, init_groups)
        # i = 0
        # best_group = deepcopy(init_groups)
        # while i < self._iterations:
        #     temp = self._initial_temperature * \
        #                   (1 - (i / (self._iterations - 1)))
        #     random_swap(init_groups, i)
        #     new_score = total_score(survey, init_groups)
        #     if accept(best_score, new_score, temp, i):
        #         best_score = new_score
        #         best_group = deepcopy(init_groups)
        # groups = []
        # for items in best_group:
        #     groups.append(Group(items))
        # grouping = Grouping()
        # for group in groups:
        #     grouping.add_group(group)
        ...
        students = slice_list(list(course.get_students()), self.group_size)
        best_score = total_score(survey, students)
        temp = self._initial_temperature
        for i in range(self._iterations):
            new_list = deepcopy(students)
            random_swap(new_list, i)
            new_score = total_score(survey, new_list)
            temp = temp * (1 - i / self._iterations)
            if accept(best_score, new_score, temp, i) and temp != 0:
                best_score = new_score
                students = new_list
        ans = Grouping()
        for student in students:
            ans.add_group(Group(student))
        return ans
