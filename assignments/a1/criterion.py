...
class HomogeneousCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        if len(answers) == 1:
            ...
            res = [(a, b) for idx, a in enumerate(answers) for b in answers[idx + 1:]]
            scores = 0

            for answer in res:
                if not answer[0].is_valid(question):
                    raise InvalidAnswerError
                if not answer[1].is_valid(question):
                    raise InvalidAnswerError
                scores += question.get_similarity(answer[0], answer[1])

            return scores/len(res)
...
class HeterogeneousCriterion(HomogeneousCriterion):
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        if len(answers) == 1:
            ...
            res = [(a, b) for idx, a in enumerate(answers) for b
                   in answers[idx + 1:]]
            scores = 0
...
class LonelyMemberCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        Preconditions:
            - len(answers) > 0
        """
        if len(answers) == 1 and answers.is_valid(question):
            return 0.0
        ...
        for answer in answers:
            if not answer.is_valid(question):
                raise InvalidAnswerError
            content_list.append(answer.content)
