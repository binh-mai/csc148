...
class Group:
    ...
    def __str__(self) -> str:
        ...
        return f'This group contains these members: {string}.'
...
class SimulatedAnnealingGrouper(Grouper):
    ...
    _initial temperature: the initial temperature to create groups within the
    group_size limit
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        init_groups = slice_list(student, self.group_size)
        best_score = total_score(survey, init_groups)
        temper = self._initial_temperature
        i = 0
        while i < self._iterations:
            group = deepcopy(init_groups)
            random_swap(group, i)
            temper = temper * (1 - i / (self._iterations - 1))
            new_score = total_score(survey, group)
            if accept(best_score, new_score, temper, i):
                best_score = new_score
                init_groups = deepcopy(group)
            i += 1
        groups = []
        for items in init_groups:
            groups.append(Group(items))
        grouping = Grouping()
        for group in groups:
            grouping.add_group(group)
