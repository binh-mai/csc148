    ...
    def score_answers(self, question: Question, answers: list[Answer]) -> float:
        ...
        for items in answers:
            if isinstance(items, list):
                for strs in items:
                    array_thing.append(strs.content)
            else:
                array_thing.append(items.content)
        appear_times = {}
        for elements in array_thing:
            if elements not in appear_times:
                appear_times[elements] = 1
            else:
                appear_times[elements] += 1
        for keys in appear_times:
            if appear_times[keys] == 1:
                return 0.0
        return 1.0
