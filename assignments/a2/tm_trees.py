...
def path_to_nested_tuple(path: str) -> tuple[str, int | list]:
    ...
    *exactly* the same, so this doctest _might_ not pass when run on
    ...
    >>> result = path_to_nested_tuple(path)
    >>> result[0]
    'prep'
    >>> result[1]
    ...
    if not os.path.isdir(path):
        ...
        return (path, [path_to_nested_tuple(os.path.join(path, f))
...
def dir_tree_from_nested_tuple(obj: tuple[str, int | list]) -> DirectoryTree:
    ...
    if isinstance(obj[1], int):
        return DirectoryTree(obj[0], obj[1])
    else:
        return DirectoryTree(obj[0], dir_tree_from_nested_tuple(obj[1]))
...
class TMTree:
    ...
    def is_displayed_tree_leaf(self) -> bool:
        ...
        if self._expanded is False:
            if self._parent_tree is None or self._parent_tree._expanded:
                return True
        return False

    def _get_parents(self) -> list[TMTree]:
        """Return a list of this tree's ancestors, starting from the root.

        >>> d1 = TMTree('C1', [], 5)
        >>> d2 = TMTree('C2', [d1], 1)
        >>> d3 = TMTree('C', [d2], 1)
        >>> d1._get_parents() == [d3, d2]
        True
        """
        if self._parent_tree is None:
            return []
        else:
            return self._parent_tree._get_parents() + [self._parent_tree]
    ...
    def get_path_string(self) -> str:
        ...
        'C6 | C5 | C4 | C3(8) None'
        >>> d1.get_path_string()
        'C6 | C5 | C4 | C3 | C2 | C1(5) None'
        >>> d2.get_path_string()
        'C6 | C5 | C4 | C3 | C2(6) None'
        """
        string = ''
        parents = self._get_parents()
        if parents is not None:
            for parent in parents:
                string += f'{parent._name} | '
                while parent._parent_tree is not None:
                    parent = parent._parent_tree
            string += f'{self._name}' + self.get_suffix()
            return string

        else:
            return f'{self._name}{self.get_suffix()}'
    ...
    def _is_leaf(self) -> bool:
        """
        Return whether this tree is a leaf.
    ...
    def update_rectangles(self, rect: tuple[int, int, int, int]) -> None:
        ...
        >>> z2 = TMTree('C2', [], 15)
        >>> z3 = TMTree('C', [z1, z2], 1)
        >>> z3.update_rectangles((0, 0, 100, 200))
        >>> z1.rect
        (0, 0, 100, 50)
        >>> z2.rect
        (0, 50, 100, 150)
        ...
        >>> a1 = TMTree('A1', [], 10)
        >>> a2 = TMTree('A2', [], 15)
        >>> a4 = TMTree('A4', [a1, a2], 5)
        >>> a3 = TMTree('A3', [], 5)
        >>> a5 = TMTree('A5', [a3], 5)
        >>> a6 = TMTree('A6', [a4, a5], 5)
        >>> a6.update_rectangles((0, 0, 200, 100))
        >>> a1.rect
        (0, 0, 60, 100)
        >>> a2.rect
        (60, 0, 90, 100)
        ...
        if self._is_leaf():
            ...
            if width > height:
                total_size = sum([subtree.data_size for subtree
                                  in self._subtrees])
                x_coord = x
                for subtree in self._subtrees:
                    subtree.update_rectangles((x_coord, y,
                                               int(subtree.data_size /
                                                   total_size * width), height))
                    x_coord += int(subtree.data_size / total_size * width)
            else:
                total_size = sum([subtree.data_size for subtree in
                                  self._subtrees])
                y_coord = y
                for subtree in self._subtrees:
                    subtree.update_rectangles((x, y_coord, width,
                                               int(subtree.data_size /
                                                   total_size * height)))
                    y_coord += int(subtree.data_size / total_size * height)
        self.rect = rect
    ...
    def get_rectangles(self) -> list[tuple[tuple[int, int, int, int],
        ...
        >>> e2 = TMTree('C2', [], 15)
        >>> e3 = TMTree('C', [e1, e2], 1)
        >>> e3.update_rectangles((0, 0, 100, 200))
        >>> rectangles = e3.get_rectangles()
        >>> rectangles[0][0]
        (0, 0, 100, 50)
        >>> rectangles[1][0]
        (0, 50, 100, 150)
        ...
        >>> a6 = TMTree('A6', [a4, a5], 5)
        >>> a6.update_rectangles((0, 0, 200, 100))
        >>> rectangles = a3.get_rectangles()
        >>> rectangles[0][0]
        (150, 0, 50, 100)
        >>> rectangles_1 = a6.get_rectangles()
        >>> rectangles_1[0][0]
        (0, 0, 60, 100)
        >>> rectangles_1[1][0]
        (60, 0, 90, 100)
        ...
        if self._is_leaf():
            ...
            return sum([subtree.get_rectangles() for subtree
                        in self._subtrees], [])
    ...
    def get_tree_at_position(self, pos: tuple[int, int]) -> Optional[TMTree]:
        ...
        True
        >>> t3.get_tree_at_position((0, 50)) is s2
        False
        >>> t3.get_tree_at_position((100, 150)) is s1
        False
        """
        if self._is_leaf():
            if self.rect[0] <= pos[0] <= self.rect[2] and self.rect[1] \
                    <= pos[1] <= self.rect[3]:
                return self
            else:
                return None
            ...
            for subtree in self._subtrees:
                if subtree.rect[0] <= pos[0] <= subtree.rect[2] and \
                        subtree.rect[1] <= pos[1] <= subtree.rect[3]:
    ...
    def collapse(self) -> TMTree:
        ...
        if not self._is_leaf():
            ...
            self._expanded = True
            return self._subtrees[0].collapse()
    ...
    def move(self, destination: TMTree) -> None:
        ...
        >>> b2 = TMTree('C2', [], 15)
        >>> b3 = TMTree('C', [b1, b2], 1)
        >>> b3.update_rectangles((0, 0, 100, 200))
        >>> b1.is_displayed_tree_leaf()
        True
        >>> b2.is_displayed_tree_leaf()
        True
        >>> b2.move(b1)
        >>> b2.rect
        (0, 0, 100, 200)
        ...
        >>> b3.data_size
        21
        >>> b3.get_tree_at_position((0, 0)) is b2
        True
        >>> b1.is_displayed_tree_leaf()
        False
        >>> b2.is_displayed_tree_leaf()
...
class FileTree(TMTree):
    ...
    def __init__(self, path: str) -> None:
        ...
        """
        super().__init__(rect, data_size, _colour, _name, _subtrees,
                         _parent_tree, _expanded)
        self._name = path
        self._subtrees = []
...
class ChessTree(TMTree):
    ...
    def __init__(self, move_dict: dict[tuple[str, int], dict],
        ...
        self._num_games_ended = num_games_ended
