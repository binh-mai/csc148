...
class User:
    ...
    def verbosity(self, y: int) -> int:
        ...
        # Find all tweets with Tweet.created_at.year() = y using a loop
        for tweet in self.tweets():
            if tweet.created_at.year() == y:
                num += len(tweet.content)  # Add length of tweet content to num
    ...
    def hack(self) -> None:
        ...
        'mwahahaha'
        """
        for user in self.follows:   # Going through followers list
            for tweet in user.tweets:  # Going through tweets of all followers
