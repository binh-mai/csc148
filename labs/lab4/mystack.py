...
def remove_big(s: Stack) -> None:
    ...
    while not s.is_empty():
        item = s.pop()
        if not item > 5:
            temporary_stack.push(item)

    while not temporary_stack.is_empty():
        s.push(temporary_stack.pop())

    # s = temporary_stack (this doesn't work because we want s to be the same
    # stack + will raise EmptyStack Error when pop s)
